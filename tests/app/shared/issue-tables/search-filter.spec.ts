...
describe('search-filter', () => {
  describe('applySearchFilter()', () => {
    ...
    it('can filter for issues which are assigned to a specific user', () => {
      ...
      expect(applySearchFilter(searchKey, displayedColumns, issuesList)).toEqual([mediumSeverityIssueWithAssigneee]);
    });

    it('can filter for issues using label', () => {
      searchKey = GITHUB_LABEL_FEATURE_FLAW.name;
      expect(applySearchFilter(searchKey, displayedColumns, issuesList)).toEqual([lowSeverityFeatureFlawIssue]);
    });
    ...
    it('can filter for issues that contain the search key in any other column', () => {
      ...
      expect(applySearchFilter(searchKey, displayedColumns, issuesList)).toEqual([mediumSeverityIssueWithResponse]);
