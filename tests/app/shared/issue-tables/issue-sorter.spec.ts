...
describe('issuer-sorter', () => {
  describe('getSortedData()', () => {
    ...
    it('sorts issues based on their updated date fields correctly', () => {
      ...
      const sortedIssuedByIdAsc = getSortedData(matSort, [issueUpdatedEarlier, issueUpdatedLater]);
      assertOrder(sortedIssuedByIdAsc, issueUpdatedEarlier, issueUpdatedLater);

      matSort.direction = 'desc';
      const sortedIssuedByIdDesc = getSortedData(matSort, [issueUpdatedEarlier, issueUpdatedLater]);
      assertOrder(sortedIssuedByIdDesc, issueUpdatedLater, issueUpdatedEarlier);
    });
  });
});
