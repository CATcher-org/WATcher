...
export class ErrorHandlingService implements ErrorHandler {
  ...
  handleError(error: HttpErrorResponse | Error | RequestError | ApolloError | RejectionErrorWrapper, actionCallback?: () => void): void {
    ...
    if (error instanceof ApolloError) {
      if (error.graphQLErrors?.length) {
        error.graphQLErrors.forEach((gqlError: GraphQLError) => {
          ...
          const message = isRateLimit ? 'GitHub API rate limit exceeded. Please try again later.' : gqlError.message;
